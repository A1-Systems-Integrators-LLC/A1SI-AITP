...
class TestSyncOrderEdgeCases:
    ...
    async def test_no_fee_in_ccxt_response(self):
        ...
        with (
            ...
            patch("trading.services.live_trading.get_channel_layer", return_value=None),
        ):
            result = await LiveTradingService.sync_order(order)
...
class TestForexPaperTradingEdgeCases:
    def test_price_none_skips_entry(self):
        ...
        with (
            patch.object(svc, "_get_price", return_value=0.0),
            patch.object(svc, "_get_open_symbols", return_value=set()),
        ):
    ...
    def test_run_cycle_returns_dict(self):
        ...
        svc = ForexPaperTradingService()
        with (
            patch.object(svc, "_check_entries", return_value=0),
            patch.object(svc, "_check_exits", return_value=0),
        ):
...
class TestForexExitLogic:
    ...
    def test_time_limit_exit(self):
        ...
        svc = ForexPaperTradingService()
        with (
            patch.object(svc, "_get_price", return_value=1.10),
            patch(
                "trading.services.forex_paper_trading.async_to_sync", return_value=lambda o: None
            ),
        ):
    ...
    def test_score_decay_exit(self):
        ...
        MarketOpportunity.objects.create(
            ...
            opportunity_type="rsi_bounce",
            score=20,
            details={},  # Below EXIT_SCORE_THRESHOLD=40
