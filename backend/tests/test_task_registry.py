...
class TestDatabaseConfiguration:
    """Verify PostgreSQL is the configured database backend."""

    @pytest.mark.django_db
    def test_database_is_postgresql(self):
        """The database backend must be PostgreSQL, never SQLite."""
        from django.db import connection

        assert connection.vendor == "postgresql", (
            f"Database vendor is '{connection.vendor}', expected 'postgresql'. "
            "SQLite was removed from this project — never revert to it."
        )
