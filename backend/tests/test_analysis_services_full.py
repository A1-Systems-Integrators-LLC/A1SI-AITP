...
class TestStepSentimentAggregate:
    def test_success(self):
        ...
        with (
            ...
            patch(
                "market.services.news.NewsService.get_sentiment_summary", return_value={"avg": 0.5}
            ),
        ):
...
class TestStepStrategyRecommend:
    def test_success(self):
        ...
        cb = MagicMock()
        with patch(
            "market.services.regime.RegimeService.get_all_recommendations",
...
class TestDataPipelineServiceList:
    def test_list_available_data(self, tmp_path):
        ...
        with (
            ...
            patch("analysis.services.data_pipeline.get_processed_dir", return_value=tmp_path),
        ):
...
class TestDataPipelineServiceDownload:
    def test_download_success(self):
        ...
        with (
            patch("analysis.services.data_pipeline.ensure_platform_imports"),
            patch("common.data_pipeline.pipeline.fetch_ohlcv", return_value=mock_df),
            patch(
                "common.data_pipeline.pipeline.save_ohlcv", return_value=Path("/tmp/out.parquet")
            ),
        ):
            cb = MagicMock()
    ...
    def test_download_empty_df(self):
        ...
        with (
            patch("analysis.services.data_pipeline.ensure_platform_imports"),
            patch("common.data_pipeline.pipeline.fetch_ohlcv", return_value=pd.DataFrame()),
        ):
            cb = MagicMock()
            result = DataPipelineService.download_data(
                {"symbols": ["BTC/USDT"], "timeframes": ["1h"]},
    ...
    def test_download_error_isolated(self):
        ...
        with (
            patch("analysis.services.data_pipeline.ensure_platform_imports"),
            patch(
                "common.data_pipeline.pipeline.fetch_ohlcv", side_effect=Exception("network error")
            ),
        ):
            cb = MagicMock()
            result = DataPipelineService.download_data(
...
class TestDataPipelineServiceGenerate:
    def test_generate_sample(self, tmp_path):
        ...
        with (
            patch("analysis.services.data_pipeline.ensure_platform_imports"),
            patch(
                "common.data_pipeline.pipeline.save_ohlcv", return_value=tmp_path / "out.parquet"
            ),
        ):
            cb = MagicMock()
            result = DataPipelineService.generate_sample_data(
                {"symbols": ["BTC/USDT"], "timeframes": ["1h"], "days": 1},
                cb,
            )
...
class TestExecuteWorkflowEdgeCases:
    ...
    def test_step_exception_fails_workflow(self):
        ...
        WorkflowStepRun.objects.create(workflow_run=run, step=step, order=1)
        with patch(
            "analysis.services.step_registry.STEP_REGISTRY",
    ...
    def test_result_chaining(self):
        ...
        step1 = WorkflowStep.objects.create(
            workflow=wf,
            name="step1",
            step_type="sentiment_aggregate",
            order=1,
        )
        step2 = WorkflowStep.objects.create(
            workflow=wf,
            name="step2",
            step_type="composite_score",
            order=2,
        )
        run = WorkflowRun.objects.create(workflow=wf, trigger="manual", params={})
        ...
        step1_result = {
            "status": "completed",
            "signal": {"signal": 0.5, "conviction": 0.8},
            "summary": {},
        }
        step2_result = {
            "status": "completed",
...
class TestWorkflowEngineCancel:
    ...
    def test_cancel_completed_fails(self):
        ...
        run = WorkflowRun.objects.create(
            workflow=wf, trigger="manual", params={}, status="completed"
        )
