"""Full coverage tests for core/services/.

Covers task_registry executors, dashboard, ws_broadcast.
...
class TestRunNewsFetch:
    def test_fetches_all_asset_classes(self):
        ...
        with (
            patch("market.services.news.NewsService", return_value=mock_svc),
...
class TestRunDataQuality:
    ...
    def test_some_failed(self):
        ...
        bad = MagicMock(
            passed=False, symbol="BTC/USDT", timeframe="1h", issues_summary=["gaps detected"]
        )
        with (
            patch(PB_ENSURE),
            patch("common.data_pipeline.pipeline.validate_all_data", return_value=[good, bad]),
        ):
            result = _run_data_quality({}, MagicMock())
...
class TestRunMlTraining:
    ...
    def test_training_failure_isolated(self):
        ...
        with patch(
            "analysis.services.ml.MLService.train", side_effect=Exception("insufficient data")
        ):
...
class TestRunMarketScan:
    def test_success(self):
        ...
        with patch("market.services.market_scanner.MarketScannerService") as mock_scanner:
    ...
    def test_scan_failure(self):
        ...
        with patch(
            "market.services.market_scanner.MarketScannerService",
...
class TestRunDailyReport:
    def test_success_with_telegram(self):
        ...
        with (
            patch("market.services.daily_report.DailyReportService") as mock_dr,
            patch("core.services.notification.NotificationService.send_telegram_sync"),
        ):
            mock_dr.return_value.generate.return_value = mock_report
    ...
    def test_telegram_failure_isolated(self):
        ...
        with (
            patch("market.services.daily_report.DailyReportService") as mock_dr,
            patch(
                "core.services.notification.NotificationService.send_telegram_sync",
                side_effect=Exception("telegram fail"),
            ),
        ):
            mock_dr.return_value.generate.return_value = {
                "regime": {},
    ...
    def test_report_generation_failure(self):
        ...
        with patch("market.services.daily_report.DailyReportService") as mock_dr:
            mock_dr.return_value.generate.side_effect = Exception("db error")
            result = _run_daily_report({}, MagicMock())
        assert result["status"] == "error"
...
class TestRunForexPaperTrading:
    def test_success(self):
        ...
        with patch("trading.services.forex_paper_trading.ForexPaperTradingService") as mock_fpt:
            mock_fpt.return_value.run_cycle.return_value = {
                "status": "completed",
                "entries_created": 1,
                "exits_created": 0,
            }
    ...
    def test_failure(self):
        ...
        with patch(
            "trading.services.forex_paper_trading.ForexPaperTradingService",
...
class TestRunHftBacktest:
    ...
    def test_successful_backtest(self):
        ...
        with (
            ...
            patch(PB_CONFIG, return_value={"data": {"watchlist": ["BTC/USDT"]}}),
            patch("hftbacktest.hft_runner.list_hft_strategies", return_value=["MarketMaker"]),
            patch("hftbacktest.hft_runner.run_hft_backtest", return_value={"total_return": 0.02}),
        ):
...
class TestRunRegimeDetection:
    def test_success(self):
        ...
        with patch("market.services.regime.RegimeService") as mock_rs:
            mock_rs.return_value.get_all_current_regimes.return_value = [
    ...
    def test_broadcast_failure_isolated(self):
        from core.services.task_registry import _last_known_regimes, _run_regime_detection

        _last_known_regimes["BTC/USDT"] = "strong_trend_up"  # Set prev regime
        with (
            patch("market.services.regime.RegimeService") as mock_rs,
            patch(
                "core.services.ws_broadcast.broadcast_regime_change",
...
class TestRunWorkflow:
    def test_delegates_to_execute_workflow(self):
        ...
        with patch(
            "analysis.services.workflow_engine.execute_workflow",
