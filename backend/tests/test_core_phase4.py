...
class TestCoreModels:
    ...
    def test_audit_log_str(self):
        ...
        log = AuditLog(user="admin", action="POST /api/test", created_at=datetime.now(tz=timezone.utc))
        s = str(log)
        assert "admin" in s
...
class TestTelegramFormatter:
    def test_order_submitted(self):
        ...
        order = MagicMock(side="buy", amount=1.5, symbol="BTC/USDT",
                          order_type="limit", exchange_id="kraken",
    ...
    def test_order_filled(self):
        from core.services.notification import TelegramFormatter
        order = MagicMock(side="sell", amount=2.0, symbol="ETH/USDT",
                          avg_fill_price=3000, fee=0.5, fee_currency="USDT",
    ...
    def test_order_filled_no_fee(self):
        from core.services.notification import TelegramFormatter
        order = MagicMock(side="buy", amount=1, symbol="X", avg_fill_price=100,
                          fee=None, exchange_order_id="O1")
        msg = TelegramFormatter.order_filled(order)
...
class TestAuthViews:
    ...
    def test_login_success(self):
        resp = self.client.post("/api/auth/login/", {"username": "testuser", "password": "pass123!"}, format="json")
    ...
    def test_login_lockout(self):
        self.client.post("/api/auth/login/", {"username": "testuser", "password": "wrong"}, format="json")
        self.client.post("/api/auth/login/", {"username": "testuser", "password": "wrong"}, format="json")
        resp = self.client.post("/api/auth/login/", {"username": "testuser", "password": "pass123!"}, format="json")
    ...
    def test_logout(self):
        self.client.post("/api/auth/login/", {"username": "testuser", "password": "pass123!"}, format="json")
...
class TestRateLimitMiddleware:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="rluser", password="pass!")
...
class TestApps:
    ...
    def test_start_scheduler_exception(self):
        from core.apps import _start_scheduler
        with patch("core.services.scheduler.get_scheduler", side_effect=RuntimeError("fail")), \
             patch("core.apps._maybe_start_order_sync"):
...
class TestValidateDepsCommand:
    ...
    def test_missing_dep_shown(self):
        stdout = StringIO()
        with patch("core.management.commands.validate_deps.importlib.import_module",
...
class TestSchedulerExecuteTask:
    ...
    def test_execute_task_paused(self):
        from core.models import ScheduledTask
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(id="paused_t", name="P", task_type="data_refresh", status="paused")
        sched = TaskScheduler()
    ...
    def test_execute_task_no_executor(self):
        from core.models import ScheduledTask
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(id="noexec_t", name="N", task_type="fake_type", status="active")
        sched = TaskScheduler()
    ...
    def test_execute_task_success(self):
        ...
        ScheduledTask.objects.create(
            id="exec_t", name="Exec", task_type="data_refresh",
            status="active", interval_seconds=300
        )
        sched = TaskScheduler()
        sched._scheduler = MagicMock()
        sched._scheduler.get_job.return_value = MagicMock(next_run_time=datetime.now(tz=timezone.utc))
    ...
    def test_execute_task_broadcast_failure_ignored(self):
        from core.models import ScheduledTask
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(id="bcast_t", name="B", task_type="data_refresh", status="active")
        sched = TaskScheduler()
        sched._scheduler = MagicMock()
...
class TestSchedulerExecuteWorkflow:
    ...
    def test_execute_workflow_disabled(self):
        ...
        Workflow.objects.create(
            id="disabled_wf", name="D", schedule_enabled=False, is_active=True
        )
        sched = TaskScheduler()
    ...
    def test_execute_workflow_success(self):
        ...
        from core.services.scheduler import TaskScheduler
        Workflow.objects.create(
            id="active_wf", name="Active", schedule_enabled=True, is_active=True
        )
        sched = TaskScheduler()
    ...
    def test_execute_workflow_exception(self):
        ...
        from core.services.scheduler import TaskScheduler
        Workflow.objects.create(
            id="fail_wf", name="Fail", schedule_enabled=True, is_active=True
        )
        sched = TaskScheduler()
        with patch("analysis.services.workflow_engine.WorkflowEngine.trigger",
...
class TestSchedulerValidateWatchlist:
    ...
    def test_validate_watchlist_invalid_symbols(self):
        ...
        with patch("core.platform_bridge.ensure_platform_imports"), \
             patch("core.platform_bridge.get_platform_config",
                    return_value={"data": {"watchlist": ["BTC/USDT", "FAKE/USD"]},
                                  "exchanges": {"kraken": {"exchange_id": "kraken"}}}), \
             patch("ccxt.kraken", return_value=mock_exchange):
...
class TestAuditLogListView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="auditor", password="pass!")
...
class TestDashboardKPIView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="dashuser", password="pass!")
        self.client.login(username="dashuser", password="pass!")

    def test_get_kpis(self):
        with patch("core.services.dashboard.DashboardService.get_kpis",
                    return_value={"portfolio": {}, "trading": {}, "risk": {},
    ...
    def test_get_kpis_with_asset_class(self):
        with patch("core.services.dashboard.DashboardService.get_kpis",
                    return_value={"portfolio": {}}) as mock_kpi:
...
class TestPlatformStatusView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="platuser", password="pass!")
        self.client.login(username="platuser", password="pass!")

    def test_get_status(self):
        with patch("core.views._get_framework_status", return_value=[]), \
...
class TestPlatformConfigView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="cfguser", password="pass!")
...
class TestNotificationPreferencesView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="notifuser", password="pass!")
...
class TestMetricsView:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="metricsuser", password="pass!")
    ...
    def test_get_metrics_with_data(self):
        ...
        RiskState.objects.create(
            portfolio_id=1, total_equity=10000.0, peak_equity=10000.0,
            is_halted=False
        )
        with patch("market.services.circuit_breaker.get_all_breakers", return_value=[
...
class TestSchedulerViews:
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        User.objects.create_user(username="scheduser", password="pass!")
    ...
    def test_scheduler_status(self):
        with patch("core.services.scheduler.get_scheduler") as mock_gs:
            mock_gs.return_value.get_status.return_value = {
                "running": True, "total_tasks": 5, "active_tasks": 4, "paused_tasks": 1
            }
...
class TestFrameworkDetailFunctions:
    ...
    def test_get_freqtrade_details_running_instance(self):
        ...
        mock_svc.get_status.return_value = {
            "running": True, "strategy": "CryptoInvestorV1", "started_at": "2026-01-01"
        }
        mock_svc.get_open_trades = AsyncMock(return_value=[{"id": 1}])
        with patch("trading.views._get_paper_trading_services", return_value={"inst1": mock_svc}), \
    ...
    def test_get_vectorbt_details_exception(self):
        from core.views import _get_vectorbt_details
        with patch("analysis.models.ScreenResult.objects", new_callable=PropertyMock,
    ...
    def test_get_nautilus_details_no_results(self):
        ...
        with patch("analysis.models.BacktestResult.objects") as mock_qs:
            ...
            mock_qs.filter.return_value.values_list.return_value.distinct.return_value.count.return_value = 0
    ...
    def test_get_nautilus_details_recent(self):
        ...
        with patch("analysis.models.BacktestResult.objects") as mock_qs:
            ...
            mock_qs.filter.return_value.order_by.return_value.values_list.return_value.first.return_value = recent
            mock_qs.filter.return_value.values_list.return_value.distinct.return_value.count.return_value = 3
            result = _get_nautilus_details()
        assert result["_status"] == "running"
    ...
    def test_get_nautilus_details_old(self):
        ...
        with patch("analysis.models.BacktestResult.objects") as mock_qs:
            ...
            mock_qs.filter.return_value.order_by.return_value.values_list.return_value.first.return_value = old
            mock_qs.filter.return_value.values_list.return_value.distinct.return_value.count.return_value = 4
            result = _get_nautilus_details()
        assert result["_status"] == "idle"
    ...
    def test_get_nautilus_details_exception(self):
        from core.views import _get_nautilus_details
        with patch("analysis.models.BacktestResult.objects", new_callable=PropertyMock,
                    side_effect=RuntimeError):
            result = _get_nautilus_details()
        assert result["_status"] == "idle"
        assert "strategies configured" in result["_status_label"]
    ...
    def test_get_hft_details_recent(self):
        ...
        with patch("analysis.models.BacktestResult.objects") as mock_qs:
            ...
            mock_qs.filter.return_value.order_by.return_value.values_list.return_value.first.return_value = recent
            mock_qs.filter.return_value.values_list.return_value.distinct.return_value.count.return_value = 2
            result = _get_hft_details()
        assert result["_status"] == "running"
    ...
    def test_get_framework_status_returns_list(self):
        ...
        with patch("core.views._get_vectorbt_details", return_value=None), \
    ...
    def test_get_framework_status_with_details(self):
        ...
        with patch("core.views._get_vectorbt_details", return_value=details), \
             patch("core.views._get_freqtrade_details", return_value=None), \
             patch("core.views._get_nautilus_details", return_value=None), \
             patch("core.views._get_hft_details", return_value=None), \
             patch("core.views._get_ccxt_details", return_value=None):
...
class TestDashboardService:
    def test_get_kpis_full(self):
        ...
        with patch.object(DashboardService, "_get_portfolio_kpis", return_value={"count": 1}), \
             ...
             patch.object(DashboardService, "_get_paper_trading_kpis", return_value={"instances_running": 0}):
    ...
    def test_get_portfolio_kpis_with_portfolio(self):
        ...
        from portfolio.models import Portfolio
        Portfolio.objects.create(name="Test", exchange_id="kraken")
        with patch("portfolio.services.analytics.PortfolioAnalyticsService.get_portfolio_summary",
                    return_value={"holding_count": 3, "total_value": 5000.0, "total_cost": 4000.0,
                                  "unrealized_pnl": 1000.0, "pnl_pct": 25.0}):
            result = DashboardService._get_portfolio_kpis()
    ...
    def test_get_trading_kpis_with_portfolio(self):
        ...
        with patch("trading.services.performance.TradingPerformanceService.get_summary",
                    return_value={"total_trades": 10, "win_rate": 60.0, "total_pnl": 500.0,
                                  "profit_factor": 1.5}):
            result = DashboardService._get_trading_kpis()
    ...
    def test_get_risk_kpis_with_portfolio(self):
        ...
        with patch("risk.services.risk.RiskManagementService.get_status",
                    return_value={"equity": 9500.0, "drawdown": 0.05, "daily_pnl": -50.0,
                                  "is_halted": False, "open_positions": 2}):
            result = DashboardService._get_risk_kpis()
    ...
    def test_get_paper_trading_kpis_no_services(self):
        from core.services.dashboard import DashboardService
        with patch("core.services.dashboard._get_paper_trading_services", create=True,
                    return_value={}):
    ...
    def test_get_paper_trading_kpis_running_instance(self):
        ...
        mock_profit = {
            "profit_all_coin": 100.0, "profit_all_percent": 2.0,
            "trade_count": 5, "closed_trade_count": 3,
            "winning_trades": 2, "losing_trades": 1,
        }
        with patch("trading.views._get_paper_trading_services",
                    return_value={"inst1": mock_svc}), \
    ...
    def test_get_platform_kpis(self):
        from core.services.dashboard import DashboardService
        with patch("core.services.dashboard.get_processed_dir") as mock_dir, \
             patch("core.services.dashboard._get_framework_list",
...
class TestPilotPreflightUncoveredLines:
    ...
    def test_preflight_json_output(self):
        stdout = StringIO()
        with patch("core.management.commands.pilot_preflight._check_frameworks",
             ...
             patch("core.management.commands.pilot_preflight._check_database",
                    return_value={"name": "DB", "status": "pass", "detail": "ok"}), \
             patch("core.management.commands.pilot_preflight._check_scheduler",
                    return_value={"name": "Sched", "status": "pass", "detail": "ok"}), \
             patch("core.management.commands.pilot_preflight._check_risk_limits",
                    return_value={"name": "Risk", "status": "pass", "detail": "ok"}), \
             patch("core.management.commands.pilot_preflight._check_kill_switch",
                    return_value={"name": "Kill", "status": "pass", "detail": "ok"}), \
             patch("core.management.commands.pilot_preflight._check_exchange_config",
                    return_value={"name": "Exch", "status": "pass", "detail": "ok"}), \
             patch("core.management.commands.pilot_preflight._check_notifications",
             ...
             patch("core.management.commands.pilot_preflight._check_portfolio",
...
class TestPilotStatusUncoveredLines:
    ...
    def test_regime_section_generic_exception(self):
        from core.management.commands.pilot_status import _regime_section
        with patch("core.platform_bridge.ensure_platform_imports", side_effect=RuntimeError("fail")):
            result = _regime_section()
    ...
    def test_print_report_healthy(self):
        ...
        report = {
            ...
            "portfolio_id": 1, "days": 1,
            "paper_trading": {"total_trades": 5, "win_rate": 60.0,
                              "total_pnl": 100.0, "profit_factor": 1.5},
            "risk": {"equity": 10000.0, "drawdown": 0.02, "daily_pnl": 50.0,
                     "is_halted": False},
            "data_quality": {"total_files": 10, "stale": 0, "gaps": 0, "passed": 10},
            "system_health": {"scheduler_running": True, "critical_alerts": 0,
                              "warning_alerts": 1, "open_breakers": []},
    ...
    def test_print_report_critical_halted(self):
        ...
        report = {
            ...
            "portfolio_id": 1, "days": 1,
            "paper_trading": {"total_trades": 0, "win_rate": 0.0,
                              "total_pnl": 0.0, "profit_factor": None},
            "risk": {"equity": 8000.0, "drawdown": 0.20, "daily_pnl": -200.0,
                     "is_halted": True, "halt_reason": "Drawdown exceeded"},
            "data_quality": {"error": "Data unavailable", "total_files": 0},
            "system_health": {"scheduler_running": False, "critical_alerts": 3,
                              "warning_alerts": 5, "open_breakers": ["kraken"]},
    ...
    def test_print_report_warning(self):
        ...
        report = {
            ...
            "portfolio_id": 1, "days": 1,
            "paper_trading": {"total_trades": 2, "win_rate": 50.0,
                              "total_pnl": -10.0, "profit_factor": 0.8},
            "risk": {"equity": 9500.0, "drawdown": 0.05, "daily_pnl": -10.0,
                     "is_halted": False},
            "data_quality": {"total_files": 5, "stale": 1, "gaps": 2, "passed": 4},
            "system_health": {"scheduler_running": True, "critical_alerts": 0,
                              "warning_alerts": 3, "open_breakers": []},
            "regime": {"regime": "RANGE"},
        }
        cmd._print_report(report)
    ...
    def test_pilot_status_json_output(self):
        stdout = StringIO()
        with patch("core.management.commands.pilot_status._paper_trading_section",
                    return_value={"total_trades": 0, "win_rate": 0, "total_pnl": 0, "profit_factor": None}), \
             patch("core.management.commands.pilot_status._risk_section",
             ...
             patch("core.management.commands.pilot_status._system_health_section",
                    return_value={"scheduler_running": True, "critical_alerts": 0,
                                  "warning_alerts": 0, "open_breakers": []}), \
...
class TestTaskRegistryUncoveredLines:
    def test_run_order_sync_timeout(self):
        ...
        Order.objects.create(
            portfolio_id=p.id, symbol="BTC/USDT", side="buy", amount=1.0,
            order_type="limit", price=50000.0, exchange_id="kraken",
            mode="live", status=OrderStatus.SUBMITTED,
            timestamp=dj_tz.now(),
        )
    ...
    def test_run_data_quality_with_failures(self):
        ...
        mock_report_fail = MagicMock(passed=False, symbol="BTC/USDT",
                                      timeframe="1h", issues_summary=["stale data"])
        cb = MagicMock()
        with patch("core.platform_bridge.ensure_platform_imports"), \
             patch("common.data_pipeline.pipeline.validate_all_data",
...
class TestSchedulerPauseResumeWithScheduler:
    ...
    def test_pause_task_with_scheduler_removes_job(self):
        ...
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(
            id="pause_me", name="P", task_type="data_refresh",
            status="active", interval_seconds=300,
        )
        sched = TaskScheduler()
        sched._scheduler = MagicMock()
    ...
    def test_resume_task_with_scheduler_adds_job(self):
        ...
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(
            id="resume_me", name="R", task_type="data_refresh",
            status="paused", interval_seconds=600,
        )
        sched = TaskScheduler()
        sched._scheduler = MagicMock()
    ...
    def test_pause_broadcast_exception_ignored(self):
        ...
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(
            id="bcast_pause", name="BP", task_type="x", status="active"
        )
        sched = TaskScheduler()
        sched._scheduler = MagicMock()
        with patch("core.services.ws_broadcast.broadcast_scheduler_event",
    ...
    def test_resume_broadcast_exception_ignored(self):
        ...
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(
            id="bcast_resume", name="BR", task_type="x", status="paused"
        )
        sched = TaskScheduler()
        with patch("core.services.ws_broadcast.broadcast_scheduler_event",
                    side_effect=RuntimeError("ws fail")):
    ...
    def test_trigger_broadcast_exception_ignored(self):
        ...
        from core.services.scheduler import TaskScheduler
        ScheduledTask.objects.create(
            id="bcast_trig", name="BT", task_type="data_refresh", status="active"
        )
        sched = TaskScheduler()
        with patch("analysis.services.job_runner.get_job_runner") as mock_jr, \
             patch("core.services.ws_broadcast.broadcast_scheduler_event",
    ...
    def test_execute_workflow_broadcast_exception_ignored(self):
        ...
        from core.services.scheduler import TaskScheduler
        Workflow.objects.create(
            id="bcast_wf", name="BW", schedule_enabled=True, is_active=True
        )
        sched = TaskScheduler()
        with patch("analysis.services.workflow_engine.WorkflowEngine.trigger",
                    return_value=("r1", "j1")), \
...
class TestTaskRegistryNautilusHFT:
    ...
    def test_nautilus_backtest_import_error(self):
        ...
        with patch("core.platform_bridge.ensure_platform_imports"), \
             patch("core.platform_bridge.get_platform_config", return_value={}):
            with patch.dict("sys.modules", {"nautilus": None, "nautilus.nautilus_runner": None}):
    ...
    def test_hft_backtest_import_error(self):
        ...
        with patch("core.platform_bridge.ensure_platform_imports"), \
             patch("core.platform_bridge.get_platform_config", return_value={}):
            with patch.dict("sys.modules", {"hftbacktest": None, "hftbacktest.hft_runner": None}):
...
class TestAppsOrderSyncWithActiveOrders:
    ...
    def test_maybe_start_order_sync_wsgi_path(self):
        ...
        with patch("trading.models.Order.objects.filter", return_value=mock_filter_result), \
             patch("asyncio.get_running_loop", side_effect=RuntimeError("No loop")), \
             patch("trading.services.order_sync.start_order_sync") as mock_start, \
...
class TestViewsPlatformConfigNoYaml:
    ...
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        if not User.objects.filter(username="yamluser").exists():
            User.objects.create_user(username="yamluser", password="pass!")
...
class TestFrameworkDetailEdgeCases:
    ...
    def test_get_ccxt_details_disconnected(self):
        ...
        with patch("market.services.exchange.ExchangeService") as MockES, \
             patch("asgiref.sync.async_to_sync") as mock_ats:
...
class TestRiskMonitoringTaskRegistry:
    ...
    def test_risk_monitoring_exception_per_portfolio(self):
        ...
        Portfolio.objects.create(name="T", exchange_id="kraken")
        cb = MagicMock()
        with patch("risk.services.risk.RiskManagementService.periodic_risk_check",
    ...
    def test_risk_monitoring_total_exception(self):
        from core.services.task_registry import _run_risk_monitoring
        cb = MagicMock()
        with patch("portfolio.models.Portfolio.objects.values_list",
...
class TestSchedulerRecoverException:
    ...
    def test_recover_stale_exception(self):
        ...
        try:
            ...
            with patch("analysis.services.job_runner.recover_stale_jobs",
...
class TestAppsOrderSyncInThread:
    ...
    def test_wsgi_thread_order_sync(self):
        ...
        Order.objects.create(
            portfolio_id=p.id, symbol="BTC/USDT", side="buy", amount=1, price=100,
            order_type="limit", status=OrderStatus.SUBMITTED, mode="live",
            timestamp=dj_tz.now(),
        )
        with patch("asyncio.get_running_loop", side_effect=RuntimeError("no loop")), \
             patch("trading.services.order_sync.start_order_sync", return_value=MagicMock()) as mock_sync, \
...
class TestHealthViewDetailedBranches:
    ...
    def test_health_job_queue_with_oldest_pending(self):
        ...
        BackgroundJob.objects.create(
            job_type="test_task", status="pending",
...
class TestMetricsViewExceptionBranches:
    ...
    def setup_method(self):
        ...
        from django.contrib.auth import get_user_model
        User = get_user_model()
        u, _ = User.objects.get_or_create(username="metricsex", defaults={"is_staff": True})
        u.set_password("pass!")
        u.save()
...
class TestFreqtradeDetailsOpenTradesException:
    ...
    def test_open_trades_exception(self):
        from core.views import _get_freqtrade_details
        mock_svc = MagicMock()
        mock_svc.get_status.return_value = {"running": True, "strategy": "CIV1", "started_at": "2026-01-01"}
        mock_svc.get_open_trades = MagicMock(side_effect=RuntimeError("api fail"))
        with patch("trading.views._get_paper_trading_services",
                    return_value={"inst1": mock_svc}):
            with patch("asgiref.sync.async_to_sync", side_effect=RuntimeError("sync fail")):
                result = _get_freqtrade_details()
...
class TestHFTDetailsNonRecentBranch:
    ...
    def test_hft_non_recent_results(self):
        from django.utils import timezone as dj_tz
        from analysis.models import BacktestResult, BackgroundJob
        from core.views import _get_hft_details
        job = BackgroundJob.objects.create(job_type="hft_backtest", status="completed")
        br = BacktestResult.objects.create(
            job=job, framework="hftbacktest", strategy_name="GridTrader",
